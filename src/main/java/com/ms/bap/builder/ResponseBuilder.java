...
public class ResponseBuilder {
    ...
    public Context selectBuildContext(SelectRequest request, String action) {
        ...
        try{
            logger.info("agent Id {} ",CommonUtil.getFinalSearchResp().values());
            Context contextFromStore= CommonUtil.getFinalSearchResp().get(request.getMessage().getOrder().getItems().get(0).getId()).getContext();
            Context context = buildCommonRequestContext(contextFromStore, action);
            return context;
        }catch(Exception e){
            logger.error("buildBuildContext error {} ",e);
        }
        return null;
    }
    ...
    public Context confirmBuildContext(ConfirmRequest request, String action) {
        ...
        try{
            logger.info("agent Id {} ",CommonUtil.getFinalSearchResp().values());
            Context contextFromStore= CommonUtil.getFinalSearchResp().get(request.getMessage().getOrder().getProvider().getAgent().getPerson().getId()).getContext();
            Context context = buildCommonRequestContext(contextFromStore, action);
            return context;
        }catch (Exception e){
    ...
    public Context buildCommonRequestContext(Context contextFromStore, String action){
        try {
            Context context = new Context();
            if (!ObjectUtils.isEmpty(context)) {
                bppUri = contextFromStore.getBppUri();
                bapId = contextFromStore.getBapId();
                bapUri = contextFromStore.getBapUri();
                bppId = contextFromStore.getBapId();
                msgId = contextFromStore.getMessageId();
                transId = contextFromStore.getTransactionId();
            }
            context.setDomain(this.domain);
            context.setAction(action);
