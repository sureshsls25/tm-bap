...
import com.ms.common.model.common.Agent;
import com.ms.common.model.common.Person;
...
public class BapSearchController {
	...
	public ResponseEntity<String> on_search(@RequestBody String body, @RequestHeader HttpHeaders httpHeaders) throws JsonProcessingException, UnknownHostException {
		...
		request.getMessage().getCatalog().getBppProviders().forEach(provider -> {
			CommonUtil.buildFinalSearchResp(provider.getAgent().getPerson().getId(), request);
		});
